...
static void sn_transmit_chars(struct sn_cons_port *port, int raw)
{
	...
	if (kfifo_is_empty(&tport->xmit_fifo) || uart_tx_stopped(&port->sc_port)) {
		/* Nothing to do. */
		ia64_sn_console_intr_disable(SAL_CONSOLE_INTR_XMIT);
		return;
	}

	/* Get contiguous data from the kfifo and transmit it */
	while (!kfifo_is_empty(&tport->xmit_fifo)) {
		xmit_count = kfifo_out_linear_ptr(&tport->xmit_fifo,
						   (unsigned char **)&start,
						   UART_XMIT_SIZE);
		if (xmit_count <= 0)
			break;
		...
		if (raw == TRANSMIT_RAW)
			result = port->sc_ops->sal_puts_raw(start, xmit_count);
		else
			result = port->sc_ops->sal_puts(start, xmit_count);
#ifdef DEBUG
		if (!result)
			DPRINTF("`");
#endif
		if (result > 0) {
			...
			kfifo_skip_count(&tport->xmit_fifo, result);
		}
		if (result < xmit_count)
			break;
	}
...
sn_sal_console_write(struct console *co, const char *s, unsigned count)
{
	...
	if (!spin_trylock_irqsave(&port->sc_port.lock, flags)) {
		 ...
		 * lock. We wait ~20 secs after the fifo level stops
		 * changing and assume the lock holder is not functional
		...
		for (counter = 0; counter < 150; mdelay(125), counter++) {
			...
			if (spin_trylock_irqsave(&port->sc_port.lock, flags)) {
				...
				unsigned int cur_len =
					kfifo_len(&port->sc_port.state->port.xmit_fifo);
				if (llen != cur_len) {
					llen = cur_len;
					counter = 0;
				}
